...
import akka.persistence.query.KeyValueChange
import akka.persistence.query.Offset
import akka.stream.javadsl.Source

/**
 * API for reading and storing key-value objects.
 *
 * For Scala API see [[akka.persistence.query.scaladsl.KeyValueStoreQuery]].
 */
trait KeyValueStoreQuery[A] extends KeyValueStore[A] {
   ...
   */
  def currentChanges(tag: String, offset: Offset): Source[KeyValueChange[A], NotUsed]
   ...
   * @return A source of change events.
   */
  def changes(tag: String, offset: Offset): Source[KeyValueChange[A], NotUsed]
}
