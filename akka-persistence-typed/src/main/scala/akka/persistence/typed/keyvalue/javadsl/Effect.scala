...
import akka.persistence.typed.keyvalue.internal.SideEffect
...
/**
 * Factory methods for creating [[Effect]] directives - how a `KeyValueBehavior` reacts on a command.
 * Created via [[KeyValueBehavior.Effect]].
 *
...
@DoNotInherit sealed class EffectFactories[State] {
   ...
   * when the `KeyValueBehavior` is created with [[KeyValueBehaviorWithEnforcedReplies]]. When
  ...
  /**
   * When [[KeyValueBehaviorWithEnforcedReplies]] is used there will be compilation errors if the returned effect
 ...
 *
 * Instances of `Effect` are available through factories [[KeyValueBehavior.Effect]].
 *
...
/**
 * [[KeyValueBehaviorWithEnforcedReplies]] can be used to enforce that replies are not forgotten.
