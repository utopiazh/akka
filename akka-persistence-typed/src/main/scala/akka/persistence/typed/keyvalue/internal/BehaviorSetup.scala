...
import akka.persistence.typed.keyvalue.internal.InternalProtocol.RecoveryTimeout
import akka.persistence.typed.keyvalue.scaladsl.KeyValueBehavior
import akka.persistence.keyvaluestore.KeyValueStoreRegistry
import akka.persistence.keyvaluestore.scaladsl.KeyValueUpdateStore
import akka.persistence.typed.PersistenceId
import akka.persistence.typed.SnapshotAdapter
import akka.util.OptionVal

/**
 * INTERNAL API: Carry state for the `KeyValueBehavior` implementation behaviors.
 */
@InternalApi
private[akka] final class BehaviorSetup[C, S](
    ...
    val emptyState: S,
    val commandHandler: KeyValueBehavior.CommandHandler[C, S],
  ...
  val keyValueStore: KeyValueUpdateStore[Any] =
    KeyValueStoreRegistry(context.system.toClassic)
      .keyValueStoreFor[KeyValueUpdateStore[Any]](settings.keyValueStorePluginId)
