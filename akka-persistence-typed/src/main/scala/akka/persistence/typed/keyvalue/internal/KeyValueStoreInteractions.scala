...
/** INTERNAL API */
@InternalApi
private[akka] trait KeyValueStoreInteractions[C, S] {
  ...
  protected def internalGet(ctx: ActorContext[InternalProtocol]): Unit = {
    ...
    ctx.pipeToSelf[GetObjectResult[Any]](setup.keyValueStore.getObject(persistenceId)) {
  ...
  protected def internalUpsert(
    ...
    ctx.pipeToSelf[Done](setup.keyValueStore.upsertObject(persistenceId, newRunningState.seqNr, value, setup.tag)) {
