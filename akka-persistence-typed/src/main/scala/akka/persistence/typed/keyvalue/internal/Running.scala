...
import akka.persistence.typed.keyvalue.scaladsl.Effect
import akka.util.unused

/**
 * INTERNAL API
 *
 * Conceptually third (of three) -- also known as 'final' or 'ultimate' -- form of `KeyValueBehavior`.
 *
...
@InternalApi private[akka] final class Running[C, S](override val setup: BehaviorSetup[C, S])
  ...
  @InternalApi private[akka] class PersistingState(
    ...
    final def onUpsertFailed(cause: Throwable): Behavior[InternalProtocol] = {
      ...
      throw new KeyValueStoreException(setup.persistenceId, currentSequenceNumber, cause)
    }
