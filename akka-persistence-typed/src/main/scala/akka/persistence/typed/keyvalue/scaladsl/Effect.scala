...
object Effect {
   ...
   * failures if a backoff supervisor strategy is defined with [[KeyValueBehavior.onPersistFailure]].
   *
   * Side effects can be chained with `thenRun`
   */
   ...
   * when the `KeyValueBehavior` is created with [[KeyValueBehavior.withEnforcedReplies]]. When
  ...
  /**
   * When [[KeyValueBehavior.withEnforcedReplies]] is used there will be compilation errors if the returned effect
...
/**
 * [[KeyValueBehavior.withEnforcedReplies]] can be used to enforce that replies are not forgotten.
