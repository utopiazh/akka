...
import akka.persistence.typed.keyvalue.internal.KeyValueBehaviorImpl.GetState
import akka.persistence.typed.keyvalue.internal.Running.WithSeqNrAccessible
import akka.persistence.keyvaluestore.scaladsl.GetObjectResult
import akka.persistence.typed.keyvalue.RecoveryCompleted
import akka.persistence.typed.keyvalue.RecoveryFailed
 ...
 * INTERNAL API
 *
 * Second (of three) behavior of a `KeyValueBehavior`.
 *
 * In this behavior the recovery process is initiated.
 * We try to obtain the state from the configured `KeyValueStore`,
...
private[akka] class Recovering[C, S](override val setup: BehaviorSetup[C, S])
    extends KeyValueStoreInteractions[C, S]
   ...
   *
   * This method throws `KeyValueStoreException` which will be caught by the internal
  ...
  private def onRecoveryFailure(cause: Throwable): Behavior[InternalProtocol] = {
    ...
    throw new KeyValueStoreException(msg, cause)
  }
